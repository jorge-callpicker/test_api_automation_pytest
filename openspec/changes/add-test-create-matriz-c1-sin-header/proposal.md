...
- Construye `src/framework/matrix.py`: parser del CSV de matriz (`;`, `utf-8-sig`), derivación de ids `V<n>`/`I<n>` desde `Código HTTP Esperado`, slug determinista y libre de colisión para las indicaciones entre paréntesis (`MTZ-<endpoint>-<campo>-<slug>`), y construcción del payload por fila aplicando la semántica `(ausente)`/`(vacío)`/serialización `String (arreglo JSON)`. Ver `design.md` para la firma completa.
...
- Declara **101 variables `MTZ-create-*`** derivadas de las celdas de la matriz (ver catálogo completo abajo). Sus slugs deben regenerarse ejecutando el `slugify` de `matrix.py` sobre el CSV al implementar, y compararse contra los ya listados aquí (ver "Riesgo" en `design.md`).
